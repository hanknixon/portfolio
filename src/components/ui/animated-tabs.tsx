...
const AnimatedTabs = ({
  ...
  return (
    <div className={cn("w-full max-w-6xl flex flex-col gap-y-4", className)}>
      <div className="flex gap-2 flex-wrap bg-[#11111198] bg-opacity-50 backdrop-blur-sm p-2 rounded-xl justify-center">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={cn(
              "relative px-4 py-2 text-sm font-medium rounded-lg outline-none transition-all duration-300 whitespace-nowrap",
              activeTab === tab.id
                ? "text-white"
          ...
          >
            {activeTab === tab.id && (
              <motion.div
                layoutId="active-tab"
                className="absolute inset-0 bg-green-400/20 border border-green-400/40 backdrop-blur-sm !rounded-lg"
      ...
      <div className="p-6 bg-[#11111198] shadow-[0_0_20px_rgba(0,0,0,0.2)] text-white bg-opacity-50 backdrop-blur-sm rounded-xl border min-h-80 h-full">
        {tabs.map(
          (tab) =>
            activeTab === tab.id && (
              <motion.div
